...
def analyze_counter(candidate: Champion, enemy_team: TeamComp, champion_pool: dict[str, Champion]) -> dict[str, object]:
    ...
    enemy_engage = sum(ch.engage for ch in enemies)
    if enemy_engage >= 12:
    ...
    enemy_durability = sum(ch.durability for ch in enemies)
    ...
    enemy_poke_count = sum(1 for ch in enemies if "poke" in ch.tags)
    ...
    enemy_dive_count = sum(1 for ch in enemies if ch.comp_scores.get("dive", 0) >= 2)
    ...
    enemy_low_mobility_count = sum(1 for ch in enemies if ch.mobility <= 3)
